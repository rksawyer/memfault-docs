...
- Chunks from a given device need to be posted to the
  ...
  - HTTP requests should not be
    ...
    be sent.
  - In case the Memfault server responds with HTTP status code `503` (Service
    Unavailable ) or `429` (Too Many Requests), the client must back off and
    retry the request after the delay specified in the `Retry-After` response
    header.
