...
Then run this command from a terminal (on Windows, WSL2 recommended):

```bash
# the first line below is the docker run command. the second line is the gdb
# command running from the current working directory, which is mounted inside
# the container ('--volume' argument)
