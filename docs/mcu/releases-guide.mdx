...
- `Software Version` / `software_version` - Each build of a "software_type" is
  expected to be uniquely identified by its "software_version" (i.e "1.0.0").
  Together the "software_type" and "software_version" uniquely identify a
  "Symbol File".
- `Build Id` - An automatically generated identifier that is based on the
  contents of the ELF file. See
  [this section for more information](/docs/mcu/symbol-file-build-ids).

### Identifying OTA Payload Binaries
