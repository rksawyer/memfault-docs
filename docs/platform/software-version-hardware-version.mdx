...
Please see the
[core/device info component](https://github.com/memfault/memfault-firmware-sdk/blob/master/components/README.md)
...
We recommend using a [SemVer](https://semver.org/) for your software version,
[see details here](#recommended-versioning-scheme).

If you do not use SemVer, we fallback to using natural sort, as implemented by
[natsort](https://pypi.org/project/natsort/) to resolve ordering.

This means that a device will not receive a previous OTA release, even if it's
deployed to the cohort (or the device cohort changes); in other words,
**downgrades are not permitted**.

To override this logic, there's a per-cohort setting _"Disable Version
Checking"_ which will always return the currently-deployed release to a device,
even if it's older (downgrades will then be possible! so be sure the affected
devices can tolerate the downgrade).
...
But natsort will apply lexical ordering (under the default settings), which may
not result in the expected OTA response
