...
### Incidents Creation and Resolution
...
### Start and end delays illustrated
...
- If no data is received after a value that crosses the threshold (like in the
  second example above), the start delay is evaluated based on the last received
  timestamp, and an incident is automatically created once it has passed.
...
- Next, optionally set up incident start and end delays. Look at the incident
...
<p align="center">
  <img
    ...
    alt="Screenshot of the Memfault web app showing the details of a specific incident and the metric payload that lasttriggered it"
  />
</p>
