...
Staged-rollout deployments can be used to make a Release available to a Cohort
incrementally.

When a staged-rollout deployment is created for a Cohort, a random subset of its
devices (amounting to the selected rollout percentage) will be marked as
_staged_ in this Cohort. Staged-rollout deployments are only applicable to
devices that are _staged_ in the Cohort.
...
The following rules apply to all active Releases (Releases that have an active
Deployment) per Cohort:
