...
Memfault allows you to enable Development Mode for a specific Device in your
Project. This mode allows you to bypass the rate limits for that Device
temporarily. This is useful for development and testing purposes. To enable
Development Mode for a Device, navigate to the Device Details page for that
Device and click the "Development Mode" tab. In here you will find detailed
information on how Server-Side Development Mode works and how to enable it for a
Device.

![The interface to enable server-side development mode for a device in the Memfault app](/img/docs/platform/server-side-dev-mode-ui.png)

Server-Side Development Mode is designed to be used in conjunction with the
...
The Developer Mode on Linux and the Development Mode on Android are used to
enable additional features in the Memfault SDKs.

The number of Devices that can be put in Development Mode simultaneously is
currently limited.
