...
### The `memfaultd` daemon, built with `coredump_plugin`
...
`coredump_plugin` is enabled by default. Read more on [how to configure which
...
Note that `IMAGE_FSTYPES_DEBUGFS` must match the `IMAGE_FSTYPE` of the image you
pass to the command.
...
<p>
  ...
  (for example in `.tar.bz2` format) contains stripped binaries and (separately)
