...
coredump. For this we need to create a new section in the linker file and use
compiler directives to place these important variables in the new section.
Memfault provides a convenience macro, `MEMFAULT_PUT_IN_SECTION()`, to help with
this.

:::note

This has implications for your startup code. For zero initialized variables you
need to explicitly zero them before use and for value initialized variables you
will need to copy the initial values into the variables before use. This is
because the default C run-time will not be aware of your special section. See
the call to `initialize_memfault_sections()` in the example below.

:::

Create sections in the linker script as shown below (highlighted) and be sure to
include the load memory address (LMA) label too.
...
Now allocate some variables to these sections. It is up to you to allocate
variables appropriately as zero initialized (.bss) or value initialized (.data).
For this example place variables from two source files into the new sections.
...
If you do nothing more you will find that these variables have indeterminate
values when you run the program. You need to initialize them in order for them
to have the correct values. The simple function shown below will ensure that the
variables are properly initialized. Be sure no accesses are made to these
variables until after this function is called.
...
files' data at specific locations or addresses. Often times microcontrollers
(MCUs) have special memory regions that have some unique characteristic you may
wish to take advantage of. Also, it is common to share regions of memory between
separate applications on one MCU like a bootloader and the production
application. Lastly, is often the case that you need to persist information
across reboots.
