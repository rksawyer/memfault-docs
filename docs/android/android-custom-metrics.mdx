...
[Built-in Metrics](android-builtin-metrics) for more details.
...
:::info Counters could be useful as either
...
:::info Distributions may we well suited to track selected metrics over time
across the fleet, as [Timeseries Metrics](/docs/platform/metrics#timeseries) -
while also useful on the Device Timeline as part of the Metric Report. :::
...
:::info Tracking time spent in selected states may be useful to track across the
fleet using [Timeseries Metrics](/docs/platform/metrics#timeseries), while
...
:::info Properties are likely to be useful as
