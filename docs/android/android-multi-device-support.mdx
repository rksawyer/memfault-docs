...
Data is bundled together in a MAR archive. This compressed file format collects
several types of data (e.g. logs, metrics, bug reports) together, so that they
can be uploaded in one file - resulting in fewer http requests.
