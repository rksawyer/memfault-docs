...
first is by bundling all data into a MAR (Memfault Archive file) file, or by
sending each piece of data individually to Memfault as it's collected from the
system.
...
This configuration would upload individual files to Memfault.
